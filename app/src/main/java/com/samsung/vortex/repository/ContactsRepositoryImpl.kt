...
import com.samsung.vortex.repository.interfaces.IContactsRepository
import com.samsung.vortex.utils.Utils

class ContactsRepositoryImpl : IContactsRepository {
    companion object {
        private var instance: ContactsRepositoryImpl? = null

        fun getInstance(): ContactsRepositoryImpl {
            if(instance == null)
                instance = ContactsRepositoryImpl()
            return instance!!
        }
    }
    ...
    override fun getContactsWithUnreadChats(): MutableLiveData<ArrayList<User>> {
        loadContactsWitUnreadChats()
        return usersWithUnreadChats
    }

    private fun loadContactsWitUnreadChats() {
        val finalUsers: ArrayList<User> = mUsers.clone() as ArrayList<User>
        for (user in mUsers) {
                ...
                if (snapshot.exists() && snapshot.value != null) {
                    ...
                    for (dataSnapshot in snapshot.children) {
                        if (dataSnapshot.hasChild(
                                VortexApplication.application.applicationContext
                                    .getString(R.string.NAME)
                            )
                        ) {
                            val user: User? = dataSnapshot.getValue(User::class.java)
                            if (!user?.uid.equals(FirebaseAuth.getInstance().uid) && contactList.contains(user?.phone_number)
                            ) mUsers.add(user!!)
                        }
                    }
    ...
    private fun loadContactListFromPhone() {
        @SuppressLint("Recycle") val phones: Cursor? =
            VortexApplication.application.applicationContext.contentResolver.query(ContactsContract.CommonDataKinds.Phone.CONTENT_URI, null, null, null, null)
        while (phones!!.moveToNext()) {
            val phone =
                phones.getString(phones.getColumnIndex(ContactsContract.CommonDataKinds.Phone.NORMALIZED_NUMBER))
            if (phone != null) {
